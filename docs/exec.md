...
In non-interactive mode, Codex does not ask for command or edit approvals. By default it runs in `danger-full-access` mode, so it can edit files and issue networked commands without additional prompts.

If you need to restrict Codex, use `codex exec --sandbox read-only` or `codex exec --sandbox workspace-write` to re-enable guardrails.
