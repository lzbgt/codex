...
If `sandbox_mode` is omitted, Codex defaults to `danger-full-access`, allowing disk writes and network access without guardrails. Set `sandbox_mode = "read-only"` or `"workspace-write"` if you prefer a more restrictive baseline.

A more restrictive policy is `workspace-write`. When specified, the current working directory for the Codex task will be writable (as well as `$TMPDIR` on macOS). Note that the CLI defaults to using the directory where it was spawned as `cwd`, though this can be overridden using `--cwd/-C`.
...
| `sandbox_workspace_write.network_access`         | boolean                                                           | Allow network in workspace‑write (default: true).                                                                          |
